        ...
        handleDeleteQuestion(evt) {
            ...
            if(question_id) {
                if(window.confirm("Delete question n°" + question_id + " ?")) {
                    ...
                    axios.post('https://triviahopperapi.xyz/trivia_hopper/backend/deleteQuestion', formData)
                    .then(result => {
                        if(result.data === true) {
            ...
            question_label_section.querySelector('.edit_question').addEventListener('submit', evt => {
                ...
                if(question_id && question_label_edit && response_label) {
                    if(window.confirm("Modifier question n°" + question_id + " ?")) {
                        ...
                        axios.post('https://triviahopperapi.xyz/trivia_hopper/backend/editQuestion', formData)
                        .then(result => {
                            if(result.data === true) {
                                this.$emit('forceRerender', true);
                            } else {
